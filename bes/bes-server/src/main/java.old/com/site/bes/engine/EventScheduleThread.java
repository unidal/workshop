...
public class EventScheduleThread extends Thread {
	private static final Log s_log = Log.getLog(EventScheduleThread.class);
	...
	public void run() {
		...
		try {
			while (true) {
				...
				stat.init();
				m_batch.clear();
				m_transporter.fetchScheduleEvents(m_batch, stat);
				...
				if (m_batch.length() > 0) {
					...
					m_transporter.updateEvents(m_batch);
				}
