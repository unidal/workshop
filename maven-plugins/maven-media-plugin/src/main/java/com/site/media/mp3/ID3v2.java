...
public class ID3v2 {
	...
	/**
	 * Remove a specific frames with a given id. A number is given to identify the
	...
	public void update() throws IOException {
		...
		if (is_changed == true) {
			...
			if (header == null
				...
				// create temp file
				write_to = com.site.media.mp3.utils.File.getTempFile("ID3", file);
			}
			...
			if (write_to != file) {
				...
				if (!write_to.renameTo(file)) {
					com.site.media.mp3.utils.File.copy(write_to.getAbsolutePath(), file
							.getAbsolutePath());
					write_to.delete();
				}
			}
