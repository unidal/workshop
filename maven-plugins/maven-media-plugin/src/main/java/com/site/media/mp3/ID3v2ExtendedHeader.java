...
public class ID3v2ExtendedHeader {
	...
	public ID3v2ExtendedHeader(InputStream in) throws IOException {
		...
		padding_size = (int) (new com.site.media.mp3.utils.Bytes(head, 6, 4).getValue());

		// read crc if present
		if (crc_present == true) {
			byte[] crc_array = new byte[4];
			in.read(crc_array);
			crc = (int) new com.site.media.mp3.utils.Bytes(crc_array).getValue();
		}
	...
	public byte[] getBytes() {
		...
		byte[] pad_byte = (new com.site.media.mp3.utils.Bytes(padding_size, 4))
				.getBytes();
		System.arraycopy(pad_byte, 0, ret, 6, 4);

		// write crc if present
		if (crc_present == true) {
			byte[] crc_byte = (new com.site.media.mp3.utils.Bytes(crc, 4)).getBytes();
