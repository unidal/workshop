...
public class MP3Properties {
	...
	/**
	 * Create a new instance connected to <code>file</code>. Properties are read
	 * immediately.
	 * 
	...
	protected void readProperties(File file) throws IOException,
		...
		bitrate = convertBitrate(getBit(third, 7), getBit(third, 6), getBit(
				third, 5), getBit(third, 4));
	 ...
	 * 
	 * @param in1 First bit to convert
	 * @param in2 Second bit to convert
	 * @return layer
	 */
	 ...
	 * @param in1 First bit to convert
	 * @param in2 Second bit to convert
	 * @param in3 Third bit to convert
	 * @param in4 4th bit to convert
	 * @return bitrate
	 */
	...
	protected int convertMode(int in1, int in2) {
		int[] convert = { MODE_STEREO, MODE_JOINT_STEREO, MODE_DUAL_CHANNEL,
				MODE_MONO };
	...
	protected int convertEmphasis(int in1, int in2) {
		int[] convert = { EMPHASIS_NONE, EMPHASIS_5015MS, EMPHASIS_ILLEGAL,
				EMPHASIS_CCITT };
		return convert[(in1 << 1) | in2];
	}
	 ...
	 * 
	 * @param id3v2_tagsize Size of ID3v2 tag
	...
	protected long calculateLength(int id3v2_tagsize) {
		...
		return (long) Math.floor((file.length() - id3v2_tagsize) / (bitrate==0?128:bitrate)
				* 0.008);
	}

	/**
	 * Check if selected bit is set in <code>input</code>. This does not really
	 * belong here, but it is needed...
	 * 
