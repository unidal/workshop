...
public class MP3File extends java.io.File {
	...
	public void setGenre(TagContent genre) throws TagFormatException {
			...
			if (brackpos != -1) {
				...
				if (Character.isDigit(genre.getTextContent().charAt(
					...
					if (brackclose == -1) {
						...
						// parse to int
						try {
							Integer par = new Integer(genre.getTextContent()
									.substring(brackpos + 1, brackclose));
							// write genre
							try {
		...
		// store frame
		try {
			ID3v2Frame add = new ID3v2Frame("TCON", v2cont, false, false,
					false, (use_compression ? ID3v2Frame.DO_COMPRESS
	...
	public void setCopyrightText(TagContent copyright)
		...
		(new TextFrameEncoding(id3v2, "TCOP", copyright, use_compression))
				.write();
	}
	...
	public void setTrack(TagContent track) throws TagFormatException {
		...
		// store frame
		try {
			ID3v2Frame add = new ID3v2Frame("TRCK", v2cont, false, false,
					false, (use_compression ? ID3v2Frame.DO_COMPRESS
							: ID3v2Frame.NO_COMPRESSION), (byte) 0, (byte) 0);
			id3v2.addFrame(add);
		} catch (ID3v2DecompressionException e) {
	...
	public void setInternetRadioStationName(TagContent name)
			throws TagFormatException {
		(new TextFrameEncoding(id3v2, "TRSN", name, use_compression)).write();
	}
	...
	public void setUniqueFileIdentifier(TagContent ufi)
		...
		(new Frame(id3v2, "UFID", build.getBytes(), true, true, use_compression))
				.write();
	}
	...
	public void setUserDefinedText(TagContent info) throws TagFormatException {
		...
		(new Frame(id3v2, "TXXX", build.getBytes(), true, true, use_compression))
				.write();
	}
	...
	public void setUserDefinedURL(TagContent link) throws TagFormatException {
		...
		(new Frame(id3v2, "WXXX", build.getBytes(), true, true, use_compression))
				.write();
	}
	...
	public void setUnsynchronizedLyrics(TagContent lyric)
		...
		(new Frame(id3v2, "USLT", build.getBytes(), true, true, use_compression))
				.write();
	}
	...
	public void setComments(TagContent comm) throws TagFormatException {
		...
		(new Frame(id3v2, "COMM", build.getBytes(), true, true, use_compression))
				.write();
	...
	public void setPicture(TagContent pic) throws TagFormatException {
		...
		ByteBuilder build = new ByteBuilder(ByteBuilder.UNICODE, 6
				+ pic.getType().length() + 1 + pic.getDescription().length()
				* 2 + pic.getBinaryContent().length);
		...
		(new Frame(id3v2, "APIC", build.getBytes(), true, true, use_compression))
				.write();
	}
	...
	public void setEncapsulatedObject(TagContent obj) throws TagFormatException {
		...
		ByteBuilder build = new ByteBuilder(ByteBuilder.UNICODE, 6
				+ obj.getType().length() + obj.getTextSubtype().length() * 2
		...
		(new Frame(id3v2, "GEOB", build.getBytes(), true, true, use_compression))
				.write();
	}
	...
	public void setTermsOfUse(TagContent use) throws TagFormatException {
		...
		ByteBuilder build = new ByteBuilder(ByteBuilder.UNICODE, 3 + use
				.getTextContent().length() * 2);
		...
		(new Frame(id3v2, "USER", build.getBytes(), true, true, use_compression))
				.write();
	}
	...
	public void setEncryptionMethodRegistration(TagContent encr)
		...
		ByteBuilder build = new ByteBuilder(ByteBuilder.NONE, 2
				+ encr.getType().length() + encr.getBinaryContent().length);
		...
		(new Frame(id3v2, "ENCR", build.getBytes(), true, true, use_compression))
				.write();
	}
	...
	public void setGroupIdentificationRegistration(TagContent grp)
		...
		ByteBuilder build = new ByteBuilder(ByteBuilder.NONE, 2
				+ grp.getType().length() + grp.getBinaryContent().length);
		...
		(new Frame(id3v2, "GRID", build.getBytes(), true, true, use_compression))
				.write();
	}
	...
	public void setPrivateData(TagContent data) throws TagFormatException {
		...
		ByteBuilder build = new ByteBuilder(ByteBuilder.NONE, 1
				+ data.getType().length() + data.getBinaryContent().length);

		build.put(data.getType());
		build.put((byte) 0);
		build.put(data.getBinaryContent());

		(new Frame(id3v2, "PRIV", build.getBytes(), true, true, use_compression))
				.write();
	}
