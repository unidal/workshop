...
public abstract class TextFrame {
	 ...
	 */
	public TextFrame(boolean encoding, ID3v2 id3v2, String type,
			TagContent content, boolean use_compression)
		...
		ByteBuilder build = new ByteBuilder((encoding ? ByteBuilder.UNICODE
				: ByteBuilder.NONE), (encoding ? content.getTextContent()
				.length() * 2 + 1 : content.getTextContent().length()));
	...
	public static TagContent read(boolean encoding, ID3v2 id3v2, String type)
		...
		try {
			byte[] v2cont = ((ID3v2Frame) (id3v2.getFrame(type).elementAt(0)))
					.getContent();
