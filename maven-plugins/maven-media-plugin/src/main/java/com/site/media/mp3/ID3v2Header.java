...
public class ID3v2Header {
	 ...
	 */
	public ID3v2Header(byte version, byte revision, boolean unsynch,
			boolean extended_header, boolean experimental, int length) {
	 ...
	 * @exception NoID3v2HeaderException If file does not contain an ID3v2
	 *                header
	...
	/**
	 * Reads header from stream <code>in</code> Header must start at file
	 * position.
	 * 
	 * @param in Stream to read from
	 * @exception ID3v2IllegalVersionException If tag has a revision higher than
	...
	public void readFromFile(InputStream in)
			throws ID3v2IllegalVersionException, NoID3v2HeaderException,
			IOException {
		...
		version = head[3];
		revision = head[4];
		if ((version & 0xff) > (ID3v2.VERSION & 0xff) && false
		...
		// Last, read size. Size is stored in 4 bits, which all have their
		// highest
		// bit set to 0 (unsynchronization)
		size = (head[9] & 0xff) + ((head[8] & 0xff) << 7)
				+ ((head[7] & 0xff) << 14) + ((head[6] & 0xff) << 21);
	...
	public static boolean isHeader(byte[] head) {
		...
		if ((head[6] & 0xff) >= 128 || (head[7] & 0xff) >= 128
				|| (head[8] & 0xff) >= 128 || (head[9] & 0xff) >= 128)
			return false;
	...
	/**
	 * @return true if unsynchronization bit is set
	 */
	...
	/**
	 * @return true if extended header is present
	 */
	...
	/**
	 * @return true if the tag should be considered experimental
	 */
	 ...
	 * 
	 * @param act True: Set experimental bit
	 */
	...
	public byte[] getBytes() {
		...
		/*
		 * byte []size_byte = (new de.vdheide.utils.Bytes(size, 4)).getBytes();
