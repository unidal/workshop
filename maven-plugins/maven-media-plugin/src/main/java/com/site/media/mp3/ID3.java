...
public class ID3 {
   ...
   public void readTag() throws NoID3TagException, IOException {
      ...
      if (checkForTag() == false) {
         ...
         if (buffer[122] == '\0') {
            track = new Byte((byte) buffer[123]);
         } else {
            track = new Byte((byte) 0);
         }
         // ouch, what a dirty cast...
         genre = new Byte((byte) buffer[124]);
      }
   ...
   public void writeTag() throws IOException {
      ...
      in.write(fillWithNills("TAG", 3));
      in.write(fillWithNills(title, 30));
      in.write(fillWithNills(artist, 30));
      in.write(fillWithNills(album, 30));
      in.write(fillWithNills(year, 4));
      in.write(fillWithNills(comment, 29));
      if (track == null) {
         in.writeByte(0);
      } else {
    ...
    */
   private byte[] fillWithNills(String str, int maxLen) throws UnsupportedEncodingException {
      byte[] result = new byte[maxLen];

      if (str != null) {
         byte[] ba = str.getBytes(encoding);

         System.arraycopy(ba, 0, result, 0, Math.min(ba.length, maxLen));
      }
