...
class ComponentsConfigurator extends AbstractResourceConfigurator {
   ...
   public List<Component> defineComponents() {
      ...
      all.add(C(RequestLifecycle.class, "mvc", DefaultRequestLifecycle.class).req(ModelManager.class,
            ActionHandlerManager.class));

      all.add(C(ParameterProvider.class, "application/x-www-form-urlencoded", UrlEncodedParameterProvider.class).is(
            PER_LOOKUP));
      all.add(C(ParameterProvider.class, "multipart/form-data", MultipartParameterProvider.class).is(PER_LOOKUP));
      return all;
   }
