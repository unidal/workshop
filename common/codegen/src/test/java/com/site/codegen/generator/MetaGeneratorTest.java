...
//   @Ignore
   public void testGenerateReportExcelMetaModel() throws Exception {
      ...
      Element root = xmlMeta.getMeta(new InputStreamReader(getClass().getResourceAsStream("eunit_excel_instruments.xml"), "utf-8"));
   ...
   public void testGenerateEunitResourceMetaModel() throws Exception {
      ...
      XMLOutputter outputter = new XMLOutputter(Format.getPrettyFormat());
      StringWriter writer = new StringWriter(8192);
      Element root = xmlMeta.getMeta(new InputStreamReader(getClass().getResourceAsStream("eunit_resource_sample.xml"), "utf-8"));
      String packageName = getClass().getPackage().getName();
      File file = new File(String.format("%s/src/test/resources/%s%s", ".", packageName.replace('.', '/'),
