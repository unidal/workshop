...
public class JdbcDataSource extends ContainerHolder implements DataSource, Initializable, LogEnabled {
   ...
   private DriverAdapterCPDS m_cpds;
   ...
   public void initialize() throws InitializationException {
      ...
      try {
         ...
         cpds.setDriver(c.getDriver());
         cpds.setUrl(c.getUrl());
         cpds.setUser(c.getUser());
         cpds.setPassword(c.getPassword());
         cpds.setMaxIdle(c.getMaximumPoolSize());
         cpds.setMaxActive(c.getMaximumPoolSize());
         cpds.setLoginTimeout((int) c.getConnectionTimeout());
         cpds.setMinEvictableIdleTimeMillis((int) c.getIdleTimeout());

         if (c.getStatementCacheSize() > 0) {
            cpds.setMaxPreparedStatements(c.getStatementCacheSize());
         ...
         m_cpds = cpds;
         m_cpds.getPooledConnection().close();
